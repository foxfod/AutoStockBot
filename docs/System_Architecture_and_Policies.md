...
1.  **익절 (Profit Taking)**
    ...
    *   **AI 조기 익절**: 수익률 **+5%** 이상 도달 시 AI가 추가 상승 여부를 판단하여 조기 매도 가능.
2.  **손절 (Stop Loss)**
    *   **절대 손절**: -3% (기본값) 도달 시 즉시 매도.
    *   **AI 리스크 관리 (Smart Stop)**:
        *   **주기**: **10분**마다 보유 종목 검사.
        *   **조건**: 수익률이 **-0.4% 미만**으로 하락한 종목 대상.
        *   **동작**: AI가 뉴스 및 차트를 재분석하여 "단순 눌림목(HOLD)"인지 "추세 이탈/악재(SELL)"인지 판단. SELL 시그널 시 즉시 매도.

3.  **오버나잇 (Overnight Hold)**
    *   **목적**: 상승 추세가 강력하거나 내일 갭상승이 유력한 종목을 보유하여 수익 극대화.
    *   **심사 시간**: 장 마감 5~10분 전 (KR 15:10, US 05:35).
    *   **조건**: AI 분석 결과 **"HOLD"** 판정 시 당일 청산 면제.
    *   **제한**: 손실이 **-3%**를 초과하는 깊은 하락 종목은 오버나잇 금지 (리스크 관리).
