...
### [20260213_010-02] AI분석 속도 및 품질 향상 (Dashboard Analysis Upgrade)
- **로직 교체**: 대시보드 'AI 종목 분석' 기능을 기존 신규 발굴용 로직(`analyze_stock`)에서 보유 종목 전용 로직(`analyze_holding_stock`)으로 교체.
- **효과**:
    - **속도**: 불필요한 매수 적합성 평가 과정을 생략하여 응답 속도 개선.
    - **품질**: 평단가 대비 손익 분석, 리스크 관리, 구체적인 홀딩/매도 전략을 포함한 마크다운 리포트 제공.

### [20260213_010-03] AI 리스크 관리 알림 추가 (AI Risk Notification)
- **문제**: AI가 손절가 도달 전(-0.4%) 리스크 분석을 수행하고 'HOLD'(보유) 결정을 내려도, 사용자에게 아무런 알림이 없어 불안감을 조성함 (-3%인데 왜 안 팔아?).
- **해결**:
    - `trade_manager.py`: AI가 'HOLD' 판단을 내릴 경우, **손실이 -1.0% 이상**이면 텔레그램으로 보유 이유와 현재 설정된 손절가 정보를 전송하도록 개선.
    - **알림 빈도 조절**: 동일 종목에 대해 30분 간격 또는 손실이 1% 더 깊어질 때만 알림을 보내도록 스로틀링(Throttling) 적용하여 스팸 방지.

    - **알림 빈도 조절**: 동일 종목에 대해 30분 간격 또는 손실이 1% 더 깊어질 때만 알림을 보내도록 스로틀링(Throttling) 적용하여 스팸 방지.

### [20260213_010-04] 대시보드 거래 중지 스위치 기능 (Trading Switches)
- **요청**: 장중 특정 국가(KR/US)의 거래를 일시 중단하거나 재개할 수 있는 기능 필요.
- **기능**:
    - **UI**: 대시보드 상단에 🇰🇷한국 / 🇺🇸미국 거래 ON/OFF 스위치 추가.
    - **동작**: 스위치 OFF 시 해당 국가의 모든 자동 매매 활동(스캔, 매수, 리스크 관리 등)이 즉시 중단됨 (로그에 "Skipping activities" 표시).
    - **영속성**: 서버 재시작 후에도 설정값이 유지되도록 `trading_state.json` 파일에 상태 저장.

### [20260213_010-04] [버그수정] 대시보드 전량 매도 기능 복구
- **문제**: 대시보드에서 '전량 매도(시장가)' 버튼 클릭 시 반응이 없거나 에러 발생.
- **원인**: `TradeManager` 클래스 내 `sell_position` 메서드 누락 및 API 엔드포인트 연결 오류.
- **수정**:
    - `trade_manager.py`: `sell_position` 메서드 신규 구현 (미국 주식은 -2% Limit Order로 즉시 체결 유도).
    - `main.py`: `sell_trade` 및 `update_trade` 엔드포인트 로직 정상화.

### [20260213_010-05] [버그수정] 대시보드 슬롯 설정 기능 복구
- **문제**: 대시보드에서 슬롯 개수가 표시되지 않고, +/- 버튼이 작동하지 않음.
- **원인**: `dashboard.html` 내 `manualSlots` 변수 초기화 로직 부재 및 서버 상태 동기화 누락.
- **수정**:
    - `dashboard.html`: `fetchState` 함수에서 `manual_slots` 데이터를 받아오도록 수정하고, `renderSlots` 함수를 구현하여 UI에 반영.
    - **참고**: 슬롯 설정값의 서버 저장(영속성) 기능은 아직 미구현 상태 (재시작 시 초기화됨). 추후 보완 예정.

### [20260213_010-06] 배포
- .

### [20260213_010-07] 구매프로세스 v2 배포
- .

### [20260213_010-08] 대시보드 Top10 수정
- 배포

### [20260214_010-01] 대시보드 수정
- 배포

### [20260214_010-02] 휴일 산정
- 배포

### [20260217_010-01] 무한 루프 현상 픽스
- 배포

### [20260217_010-02] 미국장 휴일 체크
- 배포

### [20260219_010-01] Srt버그 픽스
- 배포

### [20260219_010-02] 추가 픽스
- 배포
